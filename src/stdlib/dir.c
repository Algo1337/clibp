...
file_item_t create_file(str_T *name, size_t sz) {
...
void dirfile_Destruct(file_item_t file) {
	if(!file)
		return;
...
void dir_Destruct(dir_t d) {
	...
	if(d->items) {
		for(int i = 0; i < d->items->idx; i++) {
			...
			if(((file_item_t)d->items->arr[i])->is_file) {
				dirfile_Destruct((file_item_t)d->items->arr[i]);
				continue;
			}
